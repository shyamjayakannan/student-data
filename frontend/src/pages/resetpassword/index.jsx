...
export default function resetPassword() {
  ...
  const check = useCallback(
    (isValid, value, id) => dispatchValue({ type: id, value, valid: isValid }),
    []
  );
      ...
      try {
        ...
        if (responsedata.type === "Success") {
          const time = Math.round(
            (Date.now() - responsedata.resetPasswordEmailSent) / 1000
          );
      ...
      <Card style={{ width: "400px" }} noshadow>
        ...
        {loader ? (
          ...
          <button
            ...
            disabled={!value.email.valid || counter > 0}
          >
            {counter > 0
              ? `Resend Email in ${counter}s`
